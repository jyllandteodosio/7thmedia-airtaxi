    ...
    class Rpfc
    {
		...
		public function rpfc_recent_posts_from_category_display($atts) {
			extract( shortcode_atts( array(
                ...
                'exclude' => array() //* added for excluding posts by post id
			), $atts ) );
            ...
            //* added for excluding posts by post id
            
            //* function for converting string post id to int
            
            $exclude_array_string = array();
            
            $exclude_array_string = explode(',', $exclude);
            
            $exclude_array = array();
            
            foreach ($exclude_array_string as $each_number) {
                $exclude_array[] = (int) $each_number;
            }
            
            //* end of function
            
            $rpfc_args['post__not_in'] = $exclude_array; //* added for excluding posts by post id
			
			$rpfc_query = new WP_Query( $rpfc_args );
			
			$output = '<div class="' . $container . '">';
            //* $output .= 'exclude: ' . $exclude_array[0] . '<br/>' . $exclude_array[1] . '<br/>';
            //* check if correct ids are excluded
		...
		public function settings_page() {
			...
			echo '<table class="form-table">  
					<tbody>
                        ...
                        <tr valign="top">
							<th scope="row">' . __('Excluded Post IDs (separate Post IDs by commas)', 'wp-recent-posts-from-category') . '</th>
							<td><label for="exclude_post" style="display:block"><input type="text" name="exclude_post" id="exclude_post"></label>
							</td>
						</tr>
					</tbody>
				</table>';	//* modified for excluding posts by post id		
