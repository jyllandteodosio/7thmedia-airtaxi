...
jQuery(window).load(function() {
	...
	var	inputCategory    =   jQuery('#shortcode_category'),
		...
		inputExclude     =   jQuery('#exclude_post'),//* modified for excluding posts by post id	
	...
	jQuery('#generate_shortcode_form').submit(function(e) {
		...
		var	category    =
			...
			excerpt     =
			meta	    =
			exclude	    =//* modified for excluding posts by post id	
			container   =   ''
		;
        ...
        if(inputExclude.val() != "") {
			var exclude = ' exclude=' + inputExclude.val();
		}//* modified for excluding posts by post id	
		
		resultTitle.html('<strong>Copy and paste the shortcode below in your page, post or widget:</strong>')
		result.html('[rpfc_recent_posts_from_category' + category + children + posts + excerpt + meta + container + exclude + ']');		//* modified for excluding posts by post id	
	})
